...
public class LMouse implements MouseAlgorithm {
    ...
    public Point[] createPath(Point start, Point end) {
        ...
        int                    max = (int) Math.sqrt(Math.pow(Math.abs(end.x - start.x), 2) + Math.pow(Math.abs(end.y - start.y), 2));
        double                 m   = 0;
        if ((end.x - start.x) != 0) {
        ...
        if (Math.abs(end.x - start.x) >= Math.abs(end.y - start.y)) {
            ...
            for (int i = 0; i <= Math.abs(end.x - start.x); i++) {
                if (i == c) {
                    ps.add(new Point(start.x + f1 * i, start.y + f2 * sin((int) r1, random, rn, max, i, r4)
            ...
            for (int i = 0; i <= Math.abs(end.y - start.y); i++) {
                if (i == c) {
                    ps.add(new Point(
                            start.x + f1 * sin((int) r1, random, rn, max, i, r4) + (int) (Math.random() * 10 - 5),
                            start.y + f2 * i));
                    c += (int) (30 + Math.random() * 10);
    ...
    public Point[] addM(final Point start, final Point end, final Point[] points) {
        if (Math.abs(end.x - start.x) >= Math.abs(end.y - start.y)) {
            ...
            for (int i = 0; i < points.length; i++) {
                points[i] = new Point(points[i].x, points[i].y + (int) ((double) (points[i].x - start.x) * m));
            }
        } else {
            ...
            for (int i = 0; i < points.length; i++) {
                points[i] = new Point(points[i].x + (int) ((double) (points[i].y - start.y) * m), points[i].y);
            }
        }
        return points;
    }
